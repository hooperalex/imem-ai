...
async function transcribeVoice(audioBase64: string): Promise<string> {
  try {
...
async function synthesizeVoice(text: string): Promise<string> {
  try {
  ...
  try {
    switch (name) {
      ...
      case 'imem_voice_query': {
        ...
        const returnAudio = (args.return_audio as boolean) ?? true;
